...
def _encode_audio_worker(audio_key: str, episode_index: int, root: Path) -> Path:
...
class LeRobotDataset(torch.utils.data.Dataset):
    ...
    def save_episode(
        ...
        if (has_video_keys or has_audio_keys) and not use_batched_encoding:
            ...
            num_mics = len(self.meta.audio_keys)
            num_sources = num_cameras + num_mics
            if parallel_encoding and num_sources > 1:
                # TODO(Steven): Ideally we would like to control the number of threads per encoding such that:
                # num_cameras * num_threads = (total_cpu -1)
                with concurrent.futures.ProcessPoolExecutor(max_workers=num_sources) as executor:
                    future_to_key = {
                        executor.submit(
                    ...
                    for future in concurrent.futures.as_completed(future_to_key):
                        ...
                        try:
                            ...
                            logging.error(f"Encoding failed for {key}: {exc}")
                            raise exc
                ...
                for audio_key in self.meta.audio_keys:
                    temp_path = results[audio_key]
                    ep_metadata.update(
                        self._save_episode_audio(audio_key, episode_index, temp_path=temp_path)
                    )
            else:
                for video_key in self.meta.video_keys:
