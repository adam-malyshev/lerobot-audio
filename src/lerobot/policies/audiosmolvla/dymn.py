...
class DyMNMedium(nn.Module):
    def __init__(self, pretrained=True, device='cpu', target_embed_dim=None):
        super().__init__()
        self.device = device
        ...
        self.projector = None
        if target_embed_dim:
            # DyMN Medium output channels (width_mult=1.0) is 960
            self.projector = Projection(960, target_embed_dim)
    ...
    def forward(self, waveform_tensor: Tensor):
        """
        Args:
            waveform_tensor: (Batch, Samples)
        Returns:
            dict: { "embeddings": (Batch, EmbedDim), "predictions": (Batch, NumClasses) }
        """
        spec = self.preprocessor(waveform_tensor)
        # (Batch, n_mels, time) -> (Batch, 1, n_mels, time)
        spec = spec.unsqueeze(1)
        preds, embeddings = self.model(spec)
        
        return {"embeddings": embeddings, "predictions": preds}

    def embed_audio(self, waveform_tensor: Tensor):
        """
        Returns projected audio tokens: (Batch, SequenceLength, TargetDim)
        """
        spec = self.preprocessor(waveform_tensor)
        spec = spec.unsqueeze(1)
        
        # Get feature map: (B, 960, F, T)
        x = self.model._feature_forward(spec)
        
        # Flatten F and T: (B, 960, L) where L = F*T
        b, c, f, t = x.shape
        x = x.view(b, c, f * t).permute(0, 2, 1) # (B, L, C)
...
def preprocess_audio(audio_path: str, target_duration: float = 1.0, sample_rate: int = 32000) -> Tensor:
    """
    Loads audio, chunks/pads to target_duration.
    Returns: (1, Samples) Tensor on CPU.
    """
    if not os.path.exists(audio_path):
        raise FileNotFoundError(f"Audio file not found: {audio_path}")
        
    waveform, sr = librosa.core.load(audio_path, sr=sample_rate, mono=True)
