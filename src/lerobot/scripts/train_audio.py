...
from lerobot.policies.audiosmolvla.modeling_audiosmolvla import AudioSmolVLAPolicy
...
@dataclass
class TrainAudioPipelineConfig(TrainPipelineConfig):
    # Override policy to enforce AudioSmolVLAConfig structure for parsing
    policy: AudioSmolVLAConfig | None = None
    # New argument to specify the base SmolVLA checkpoint path (local or Hub ID)
    base_policy_path: str | None = None


@parser.wrap()
def train(cfg: TrainAudioPipelineConfig, accelerator: Accelerator | None = None):
    cfg.validate()
    ...
    if cfg.base_policy_path and not cfg.resume:
        if is_main_process:
            logging.info(f"Loading base policy weights from {cfg.base_policy_path}")

        # Check if local path
        local_path = Path(cfg.base_policy_path)
        weights_path = None

        if local_path.exists():
            if (local_path / "model.safetensors").exists():
                weights_path = local_path / "model.safetensors"
            elif (local_path / "pretrained_model" / "model.safetensors").exists():
                weights_path = local_path / "pretrained_model" / "model.safetensors"
        ...
        if weights_path is None:
            try:
                if is_main_process:
                    logging.info(
                        f"Path not found locally, trying to download {cfg.base_policy_path} from Hub..."
                    )
                model_path = hf_hub_download(
                    repo_id=str(cfg.base_policy_path), filename="model.safetensors"
                )
                weights_path = Path(model_path)
            except Exception as e:
                # If local path provided but file missing, or hub download failed
                pass

        if weights_path and weights_path.exists():
            state_dict = load_file(weights_path)
            # Strict=False to ignore missing audio keys and extra keys if any
            missing, unexpected = policy.load_state_dict(state_dict, strict=False)
            if is_main_process:
                logging.info(f"Base weights loaded from {weights_path}")
                logging.info(f"Missing keys (expected for audio parts): {len(missing)}")
                logging.info(f"Unexpected keys: {len(unexpected)}")
                if len(missing) > 0:
                    logging.debug(f"Missing keys: {missing}")
        else:
            raise FileNotFoundError(
                f"Could not find model.safetensors in {cfg.base_policy_path} (checked local and Hub)"
            )
    ...
    preprocessor, postprocessor = make_audiosmolvla_pre_post_processors(
        config=cfg.policy, dataset_stats=dataset.meta.stats
    )
